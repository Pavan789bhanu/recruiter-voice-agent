...
# ── Model-driven end of call ──────────────────────────────────────────────────

def test_end_call_marker_strips_and_flags():
    """generate_response should strip the silent marker and raise call_complete."""
    import ai_engine
    s = ai_engine.CallSession("CAx")
    # Stub the Claude call to return a closing line + the silent marker.
    s.history.append({"role": "user", "content": "thanks, bye"})
    import unittest.mock as mock
    fake = mock.Mock()
    fake.content = [mock.Mock(text="Thank you, take care! [[END_CALL]]")]
    with mock.patch.object(ai_engine.client.messages, "create", return_value=fake):
        spoken = s.generate_response()
    assert "[[END_CALL]]" not in spoken          # marker never spoken
    assert spoken == "Thank you, take care!"
    assert s.call_complete is True


def test_call_complete_waits_then_hangs_up_if_silent():
    """On end-signal, the bot says goodbye, waits the closing grace, then hangs up."""
    import call_handler as ch
    orch = make_orchestrator()
    orch.caller_type = "ai"
    orch.session.generate_response = lambda: "Take care!"
    orch.session.call_complete = True

    spoken, hung = [], {"called": False}
    async def fake_speak(t): spoken.append(t)
    async def fake_hangup(): hung["called"] = True
    orch._speak = fake_speak
    orch._hangup = fake_hangup

    prev = ch.CLOSING_GRACE_SEC
    ch.CLOSING_GRACE_SEC = 0.05
    try:
        async def scenario():
            await orch._generate_and_play("appreciate it, talk soon")
            assert orch._closing is True
            assert hung["called"] is False     # not hung up immediately
            await asyncio.sleep(0.12)           # let the closing grace elapse
            return hung["called"]
        result = asyncio.run(scenario())
    finally:
        ch.CLOSING_GRACE_SEC = prev

    assert spoken == ["Take care!"]
    assert result is True


def test_caller_goodbye_cancels_premature_close():
    """If the caller speaks during the closing grace, we do NOT hang up on them."""
    import call_handler as ch
    orch = make_orchestrator()
    orch.caller_type = "ai"
    orch.session.generate_response = lambda: "Take care!"
    ...
    hung = {"called": False}
    async def fake_speak(t): pass
    async def fake_hangup(): hung["called"] = True
    orch._speak = fake_speak
    orch._hangup = fake_hangup

    prev = ch.CLOSING_GRACE_SEC
    ch.CLOSING_GRACE_SEC = 0.1
    try:
        async def scenario():
            await orch._generate_and_play("thanks")
            await asyncio.sleep(0.03)
            await orch._on_transcript("yeah thank you so much")  # caller's goodbye
    ...
    assert result is False     # caller spoke — not dropped


def test_idle_timer_fires_hangup():
    orch = make_orchestrator()
    hung = {"called": False}
    ...
    async def scenario():
        orch._arm_idle_hangup(0.05)        # tiny window
        await asyncio.sleep(0.12)          # let it elapse
        return hung["called"]

    assert asyncio.run(scenario()) is True


def test_caller_speech_cancels_idle_hangup():
    orch = make_orchestrator()
    hung = {"called": False}
    ...
    async def scenario():
        orch._arm_idle_hangup(0.1)
        await asyncio.sleep(0.02)
        await orch._on_transcript("yeah I'm still here")   # caller speaks -> cancel
        await asyncio.sleep(0.15)
        return hung["called"]

    assert asyncio.run(scenario()) is False   # never hung up
